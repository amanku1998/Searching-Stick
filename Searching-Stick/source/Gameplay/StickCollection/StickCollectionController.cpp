...
namespace Gameplay 
{
	namespace Collection 
	{
		...
		{
			int left = 0;		// initialize left index to the start of the collection
			int right = sticks.size();	// initialize right index to the size of the collection which is the end
		...
		void Gameplay::Collection::StickCollectionController::searchElement(SearchType search_type)
		{
			...
			case Gameplay::Collection::SearchType::BINARY_SEARCH:
				sortElements();		// sort the collection before the binary search performs
